  ...
  return (
    <main className="relative flex min-h-[100dvh] flex-col justify-center gradient-galaxy px-6">
      ...
      <form onSubmit={onSubmit} className="relative z-10 flex flex-col gap-4">
        <Input label={t("email")} type="email" placeholder={t("emailPlaceholder")} value={email} onChange={(e) => setEmail(e.target.value)} />
        <Input label={t("password")} type="password" placeholder={t("passwordPlaceholder")} value={password} onChange={(e) => setPassword(e.target.value)} error={error} />
        ...
        <Link href="/signup" className="text-center text-sm text-ink-soft">
          {t("toSignup")} <span className="font-semibold text-accent">{t("signupLink")}</span>
        </Link>
      </form>
    </main>
  );
