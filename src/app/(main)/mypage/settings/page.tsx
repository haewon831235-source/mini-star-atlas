      ...
      <div className="relative z-10 space-y-5 px-4 py-5">
        ...
        <Card className="flex items-center gap-3 p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-ink">{t("apiStatus")}</p>
            <p className="text-xs text-ink-muted">{hasKey ? `${t("connected")} · ${masked}` : t("notSet")}</p>
          </div>
        </Card>

        {/* API 키 입력 */}
        <Card className="space-y-3 p-4">
          <p className="text-sm font-bold text-ink">{t("apiSection")}</p>
          ...
          <div className="flex gap-2">
            <Button className="flex-1" onClick={save}>{t("saveBtn")}</Button>
            {hasKey && (
              <Button variant="outline" onClick={remove}>{t("deleteBtn")}</Button>
            )}
          </div>
          <a
          ...
          >
            {t("howToGet")} <ExternalLink size={12} />
          </a>
        </Card>
        ...
        <Card className="space-y-2 p-4 text-xs leading-relaxed text-ink-muted">
          <p>🔒 {t("apiNote")}</p>
          <p>✨ 키를 등록하면 <b className="text-ink-soft">오늘의 운세 AI 풀이</b>, <b className="text-ink-soft">궁합 AI 코멘트</b>{t("featureNote")}</p>
